...
class BaseScheduleStrategy(object):
    ...
    def simulate(self):
        ...
        if self.send_mail == 1:
            mail_service.send_text_mail(to_addr=['lxb_sysu@163.com'],
                                        subject='%s: %s, %s, %s' % (self.strategy_name, self.period, self.stock_amount, self.init_cap), plain_text=self.mail_content)
    ...
    def buy(self, target_list, date):
        ...
        for target in target_list:
            ...
            if r.count() == 1:
                ...
                self.balance -= buy_amount * buy_price
    ...
    def summary(self, date):
        ...
        for item in self.hold_stocks.items():
            ...
            ss += '    %s: %s: %s' % (code, amount, amount * price)
        self.total_cap = self.balance + self.market_cap
